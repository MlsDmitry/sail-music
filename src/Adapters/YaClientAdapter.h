...
struct play_audio_request_t
{
    ...
    play_audio_request_t (
            ...
            std::string from,
            std::string from_cache,
            std::string play_id,
            std::string uid,
            std::string timestamp,
            std::int64_t track_length_seconds,
            std::int64_t track_played_seconds,
            std::int64_t end_position_seconds,
            std::string playlist_id,
            std::string client_now,
            std::int64_t timeout
            )
    {
        _track_id = track_id;
    ...
    {
        io & json_dto::mandatory("track-id", _track_id)
            & json_dto::mandatory("album-id", _album_id)
            & json_dto::mandatory("from", _from)
            & json_dto::optional_no_default("from-cache", _from_cache)
            & json_dto::optional_no_default("play-id", _play_id)
            & json_dto::optional_no_default("uid", _uid)
